...
// Mock Bun.sleep to resolve immediately so exponential backoff doesn't hang tests
...
describe("search_pubmed", () => {
	test("returns articles with interpretation", async () => {
		...
		expect((result.data as any)?.model_used).toBe(true);
	});
...
describe("search_clinical_trials", () => {
	test("returns trials with interpretation", async () => {
		...
		const ctx = createMockContext();
		const result = await searchClinicalTrials.execute(
			{ query: "diabetes" },
			ctx,
		);
		expect(result.success).toBe(true);
