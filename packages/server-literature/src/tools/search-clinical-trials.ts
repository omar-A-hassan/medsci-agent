...
export const searchClinicalTrials = defineTool({
  ...
  schema: z.object({
    query: z.string().min(1).describe("Search query: condition name, drug name, or NCT number"),
    status: z.enum(["RECRUITING", "COMPLETED", "ACTIVE_NOT_RECRUITING", "ANY"]).optional()
      .describe("Filter by trial status (default: ANY)"),
    max_results: z.number().int().positive().max(25).optional().describe("Max results (default: 10)"),
  }),
  execute: async (input, ctx) => {
    ...
    if (!res.ok) {
      return { success: false, error: `ClinicalTrials.gov API returned ${res.status}` };
    }
    ...
    const trialData = { query: input.query, n_results: results.length, results };
