...
export const analyzeMedicalImage = defineTool({
	...
	schema: z.object({
		image_path: z
			.string()
			.min(1)
			.describe("Path to the medical image file (PNG, JPEG)"),
		modality: z
			.enum(["chest_xray", "pathology", "dermatology", "ct", "mri", "other"])
			.describe("Type of medical image"),
		clinical_context: z
			.string()
			.optional()
			.describe(
				"Brief clinical context or question (e.g. 'rule out pneumonia')",
			),
	}),
	execute: async (input, ctx) => {
		...
		const modalityPrompts: Record<string, string> = {
			...
			other:
				"Analyze this medical image. Describe what you observe and any notable findings.",
		};
		...
		let model_used = true;
		try {
			analysis = await ctx.ollama.generateJson<Record<string, unknown>>(
				prompt,
				{
					system: systemPrompt,
					temperature: 0.2,
			...
			try {
				...
				analysis = {
					...
					disclaimer:
						"AI-assisted analysis — not a substitute for professional medical interpretation.",
				};
			} catch {
				analysis = {
					findings: [],
