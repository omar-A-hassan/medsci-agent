...
export const searchPdb = defineTool({
  ...
  schema: z.object({
    query: z.string().min(1).describe("Search query: protein name, gene, or PDB ID (e.g. '4HHB')"),
  ...
  execute: async (input, ctx) => {
    ...
    const json = (await res.json()) as { result_set: Array<{ identifier: string; score: number }> };
