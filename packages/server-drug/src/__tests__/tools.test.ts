...
import {
	TXGEMMA_MODEL,
	parseBinaryPrediction,
	predictAdmet,
} from "../tools/predict-admet";
...
describe("analyze_molecule", () => {
	...
	test("returns error for invalid SMILES", async () => {
		const ctx = createMockContext({
			pythonResponse: { valid: false, error: "Invalid SMILES string" },
		});
		const result = await analyzeMolecule.execute(
	...
	test("calls python sidecar with correct method", async () => {
		const ctx = createMockContext({
			pythonResponse: { valid: true, canonical_smiles: "C" },
		});
		await analyzeMolecule.execute({ smiles: "C" }, ctx);
		expect(ctx.python.call).toHaveBeenCalledWith("rdkit.mol_from_smiles", {
			smiles: "C",
		});
	});
});
...
describe("lipinski_filter", () => {
	...
	test("fails non-drug-like molecule", async () => {
		...
		const result = await lipinskiFilter.execute(
			{ smiles: "LARGE_MOLECULE" },
			ctx,
		);
		expect(result.success).toBe(true);
...
describe("molecular_similarity", () => {
	...
	test("handles invalid molecule in pair", async () => {
		const ctx = createMockContext({
			pythonResponse: { error: "One or both SMILES are invalid" },
		});
		const result = await similaritySearch.execute(
...
describe("predict_admet", () => {
	...
	test("returns error for invalid SMILES", async () => {
		const ctx = createMockContext({
			pythonResponse: { valid: false, error: "Invalid SMILES" },
		});
...
describe("search_chembl", () => {
	test("searches targets and returns interpretation", async () => {
		...
		const ctx = createMockContext();
		const result = await searchChembl.execute(
			{ query: "EGFR", search_type: "target" },
			ctx,
		);
		expect(result.success).toBe(true);
