...
export const TXGEMMA_MODEL =
	"hf.co/matrixportalx/txgemma-2b-predict-GGUF:Q4_K_M";
 ...
 */
export function parseBinaryPrediction(
	raw: string,
	...
	if (
		trimmed === positiveClass ||
		trimmed.startsWith(`${positiveClass} `) ||
		trimmed.startsWith(`${positiveClass}\n`)
	)
		return true;
	if (
		trimmed === negativeClass ||
		trimmed.startsWith(`${negativeClass} `) ||
		trimmed.startsWith(`${negativeClass}\n`)
	)
		return false;
...
export const predictAdmet = defineTool({
	...
	execute: async (input, ctx) => {
		...
		if (txgemma_used) {
			...
			admet = {
				bbb_penetration:
					(props.tpsa ?? 0) < 90 && (props.molecular_weight ?? 0) < 450
						? "likely"
						: "unlikely",
