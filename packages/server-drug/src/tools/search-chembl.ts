...
export const searchChembl = defineTool({
  ...
  schema: z.object({
    query: z.string().min(1).describe("Search query: target name, molecule name, or SMILES for similarity"),
    search_type: z.enum(["target", "molecule", "similarity"]).optional()
  ...
  execute: async (input, ctx) => {
    ...
    const items = (json as any)[searchType === "target" ? "targets" : "molecules"] ?? [];
    ...
    const chemblData = { search_type: searchType, query: input.query, n_results: results.length, results };
