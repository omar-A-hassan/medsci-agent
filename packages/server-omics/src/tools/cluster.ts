...
export const cluster = defineTool({
	...
	schema: z.object({
		path: z
			.string()
			.min(1)
			.describe("Path to the preprocessed input .h5ad file"),
		output_path: z
			.string()
			.min(1)
			.describe("Path to save the clustered .h5ad file"),
		resolution: z
			.number()
			.positive()
			.optional()
			.describe("Clustering resolution (default: 1.0, higher = more clusters)"),
		method: z
			.enum(["leiden", "louvain"])
			.optional()
			.describe("Clustering algorithm (default: leiden)"),
	}),
